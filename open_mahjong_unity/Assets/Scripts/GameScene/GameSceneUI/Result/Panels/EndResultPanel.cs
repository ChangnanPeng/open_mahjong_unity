...
public class EndResultPanel : MonoBehaviour {
    ...
    public IEnumerator CoPlayJianzhongSettleHuRoutine(int huScore, string[] huFan, bool isFinalPanel) {
        if (showResultCoroutine != null) {
            StopCoroutine(showResultCoroutine);
            showResultCoroutine = null;
        }
        showResultCoroutine = StartCoroutine(PlayJianzhongSettleHuRoutine(huScore, huFan, isFinalPanel));
        yield return showResultCoroutine;
        showResultCoroutine = null;
    }

    private IEnumerator PlayJianzhongSettleHuRoutine(int huScore, string[] huFan, bool isFinalPanel) {
    ...
    private void ShowTotalPanel(string rule, int huScore, string[] huFan, int? baseFu, RiichiEndResultExtras riichiExtras = null) {
        ...
        bool isJianzhong = rule != null && rule.StartsWith("jianzhong");
