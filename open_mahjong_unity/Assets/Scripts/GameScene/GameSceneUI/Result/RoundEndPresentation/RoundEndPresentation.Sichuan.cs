...
public partial class RoundEndPresentation {
    ...
    public void EnqueueSichuanSettleHu(
        ...
        if (NormalGameStateManager.Instance != null && NormalGameStateManager.Instance.UsesWinnerResultSequence()) {
            Debug.Log($"[JianzhongEndgameQueue] enqueue settle_hu player={hepaiPlayerIndex}, hu_class={huClass}, fan_count={(huFan == null ? 0 : huFan.Length)}, final={isFinalPanel}");
        }
    ...
    private IEnumerator CoSichuanSettleHuStep(
        ...
        bool isFinalPanel) {
        if (NormalGameStateManager.Instance != null && NormalGameStateManager.Instance.UsesWinnerResultSequence()) {
            Debug.Log($"[JianzhongEndgameQueue] begin settle_hu panel player={hepaiPlayerIndex}, final={isFinalPanel}");
        }
        ...
        if (NormalGameStateManager.Instance != null && NormalGameStateManager.Instance.ScoreDisplayMultiplier() != 1) {
            yield return EndResultPanel.Instance.CoPlayJianzhongSettleHuRoutine(huScore, huFan, isFinalPanel);
            Debug.Log($"[JianzhongEndgameQueue] end settle_hu panel player={hepaiPlayerIndex}, final={isFinalPanel}");
        } else {
