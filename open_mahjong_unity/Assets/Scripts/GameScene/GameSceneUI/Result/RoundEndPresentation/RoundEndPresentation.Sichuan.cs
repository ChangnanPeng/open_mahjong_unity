...
public partial class RoundEndPresentation {
    ...
    public void EnqueueSichuanSettleHu(
        ...
        if (NormalGameStateManager.Instance != null && NormalGameStateManager.Instance.UsesWinnerResultSequence()) {
            Debug.Log($"[JiandanEndgameQueue] enqueue settle_hu player={hepaiPlayerIndex}, hu_class={huClass}, fan_count={(huFan == null ? 0 : huFan.Length)}, final={isFinalPanel}");
        }
    ...
    private IEnumerator CoSichuanSettleHuStep(
        ...
        bool isFinalPanel) {
        if (NormalGameStateManager.Instance != null && NormalGameStateManager.Instance.UsesWinnerResultSequence()) {
            Debug.Log($"[JiandanEndgameQueue] begin settle_hu panel player={hepaiPlayerIndex}, final={isFinalPanel}");
        }
        ...
        if (NormalGameStateManager.Instance != null && NormalGameStateManager.Instance.ScoreDisplayMultiplier() != 1) {
            yield return EndResultPanel.Instance.CoPlayJiandanSettleHuRoutine(huScore, huFan, isFinalPanel);
            Debug.Log($"[JiandanEndgameQueue] end settle_hu panel player={hepaiPlayerIndex}, final={isFinalPanel}");
        } else {
