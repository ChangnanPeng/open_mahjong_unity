...
public class GameStateNetworkManager : MonoBehaviour {
    ...
    private void HandleShowResult(Response response) {
        ...
        if (NormalGameStateManager.Instance != null
            && (NormalGameStateManager.Instance.IsSichuanRule() || NormalGameStateManager.Instance.IsJiandan())) {
    ...
    public async void SendChineseGameTile(bool cutClass, int tileId, int cutIndex) {
        if (NormalGameStateManager.Instance != null && NormalGameStateManager.Instance.IsRealtimeSpectator) return;
        try {
            bool isJiandan = IsJiandanActive();
            object request = isJiandan
                ? new {
    ...
    private static bool IsJiandanActive() {
        ...
        return manager.roomRule == "jiandan" || (!string.IsNullOrEmpty(manager.subRule) && manager.subRule.StartsWith("jiandan"));
    }
