...
async def new_rule_bot_action(
    ...
    try:
        ...
        if getattr(player, "user_id", None) == 2:
            await smart_new_rule_bot_action(game_state, player_index, action_list, game_status, action_tick)
        else:
            await auto_new_rule_bot_action(game_state, player_index, action_list, game_status, action_tick)
    except asyncio.CancelledError:
        raise
    except Exception as exc:
        logger.error("New-rule bot action failed: player_index=%s error=%s", player_index, exc, exc_info=True)


async def auto_new_rule_bot_action(
    game_state: Any,
    player_index: int,
    action_list: list[str],
    game_status: str,
...
async def smart_new_rule_bot_action(
    game_state: Any,
    player_index: int,
    action_list: list[str],
    game_status: str,
...
async def _submit_if_current(
    ...
    *,
    ...
    try:
        ...
        logger.info("New-rule bot submitted a stale action: player_index=%s action=%s", player_index, action_type)
        return False
    return True
