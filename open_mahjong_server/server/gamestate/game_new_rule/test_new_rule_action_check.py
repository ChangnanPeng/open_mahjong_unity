...
from server.gamestate.game_new_rule.action_check import (
...
def prepared_game() -> NewRuleGameState:
...
def test_refresh_waiting_tiles_from_new_rule_calculator() -> None:
    game = prepared_game()
...
def run() -> None:
    tests = [
        ...
        test_refresh_waiting_tiles_from_new_rule_calculator,
    ]
    for test in tests:
        test()
    print(f"new_rule action_check tests ok: {len(tests)} tests")
