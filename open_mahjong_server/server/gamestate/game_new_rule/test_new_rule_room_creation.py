...
def _server():
    ...
    async def create_NewRule_room(*args, **kwargs):
        return await server.room_manager.create_NewRule_room(*args, **kwargs)
    ...
    server.create_NewRule_room = create_NewRule_room
...
def _create_message() -> dict:
    return {
        "type": "room/create_NewRule_room",
        "roomname": "new rule smoke",
        "gameround": 1,
        "password": "",
        "roundTimerValue": 0,
        "stepTimerValue": 0,
...
async def _started_room_with_responder():
    ...
    response = await server.room_manager.create_NewRule_room(
        "conn-101",
        "new rule smoke",
        1,
        "",
    ...
    assert isinstance(game_state, NewRuleGameState)
...
async def _send_host_cut(server, websocket, game_state: NewRuleGameState, tile: int) -> None:
...
async def _wait_for_status(game_state: NewRuleGameState, status: str, player_index: int | None = None) -> None:
    for _ in range(20):
...
def test_room_manager_creates_hidden_new_rule_room() -> None:
    async def scenario() -> None:
        ...
        room_info = server.room_manager.rooms[response.room_info["room_id"]]
        assert room_info["room_rule"] == "new_rule"
        assert room_info["sub_rule"] == "new_rule/standard"
        assert room_info["hepai_limit"] == 0
        assert room_info["open_cuohe"] is False
        assert room_info["hidden_room"] is True
        assert room_info["allow_spectator"] is False
...
def test_hidden_new_rule_room_is_filtered_from_public_room_list() -> None:
    async def scenario() -> None:
        ...
        assert all(room["room_id"] != room_id for room in room_list_response.room_list)
        ...
        assert sync_response.room_info["hidden_room"] is True

    asyncio.run(scenario())


def test_hidden_new_rule_room_rejects_public_join_by_room_id() -> None:
    async def scenario() -> None:
        server, _websocket, _player = _server()
        ...
        assert join_response.type == "error_message"
        room_info = server.room_manager.rooms[room_id]
        assert room_info["hidden_room"] is True
        assert room_info["player_list"] == [101]
        assert server.players["conn-102"].current_room_id is None

    asyncio.run(scenario())


def test_hidden_new_rule_room_bot_changes_require_host() -> None:
    async def scenario() -> None:
        server, _websocket, _player = _server()
        _add_connected_player(server, 102, "conn-102")
        ...
        blocked_response = await server.room_manager.add_bot_to_room("conn-102", room_id)
        assert blocked_response.success is False
        assert server.room_manager.rooms[room_id]["player_list"] == [101]

        blocked_smart_response = await server.room_manager.add_smart_bot_to_room("conn-102", room_id)
        assert blocked_smart_response.success is False
        assert server.room_manager.rooms[room_id]["player_list"] == [101]

        host_response = await server.room_manager.add_bot_to_room("conn-101", room_id)
        assert host_response.success is True
        assert server.room_manager.rooms[room_id]["player_list"] == [101, 0]

    asyncio.run(scenario())


def test_hidden_new_rule_room_start_requires_host() -> None:
    async def scenario() -> None:
        server, _websocket, _player = _server()
        _add_connected_player(server, 102, "conn-102")
...
def test_hidden_new_rule_room_rejects_non_member_ready_and_non_host_kick() -> None:
    async def scenario() -> None:
        server, _websocket, _player = _server()
        _add_connected_player(server, 102, "conn-102")
...
def test_room_manager_forces_hidden_new_rule_spectator_off() -> None:
    async def scenario() -> None:
        server, _websocket, _player = _server()
...
def test_game_server_hidden_new_rule_create_wrapper_forces_spectator_off() -> None:
    async def scenario() -> None:
        server, _websocket, _player = _real_game_server()

        response = await server.create_NewRule_room(
            "conn-101",
            "new rule smoke",
            1,
            "",
...
def test_hidden_new_rule_smoke_uses_game_server_connection_session() -> None:
    async def scenario() -> None:
        server = GameServer()
        server.db_manager = FakeDbManager()
        server.match_manager = FakeMatchManager()
...
def test_hidden_new_rule_smoke_uses_four_game_server_connection_sessions() -> None:
    async def scenario() -> None:
        server = GameServer()
        server.db_manager = FakeDbManager()
        server.match_manager = FakeMatchManager()
...
def test_room_router_handles_hidden_new_rule_create_message() -> None:
    async def scenario() -> None:
        server, websocket, player = _server()
...
def test_hidden_new_rule_protocol_dispatch_create_start_cut_smoke() -> None:
    async def scenario() -> None:
        server, websocket, player = _server()
...
def test_hidden_new_rule_room_can_start_game_state() -> None:
    async def scenario() -> None:
        server, websocket, player = _server()
        response = await server.room_manager.create_NewRule_room(
            "conn-101",
        ...
        game_state = server.gamestate_manager.get_game_state_by_room_id(room_info["room_id"])
        assert isinstance(game_state, NewRuleGameState)
        assert game_state.room_rule == "new_rule"
...
def test_hidden_new_rule_room_accepts_first_cut_message() -> None:
    async def scenario() -> None:
        server, websocket, _player = _server()
        response = await server.room_manager.create_NewRule_room(
            "conn-101",
...
def test_hidden_new_rule_room_accepts_discard_win_response_message() -> None:
    async def scenario() -> None:
        server, websocket, _player = _server()
        response_websocket, response_player = _add_connected_player(server, 102, "conn-102")
        response = await server.room_manager.create_NewRule_room(
...
def test_hidden_new_rule_room_accepts_discard_pass_response_message() -> None:
    async def scenario() -> None:
        server, websocket, _player = _server()
        response_websocket, response_player = _add_connected_player(server, 102, "conn-102")
        response = await server.room_manager.create_NewRule_room(
...
def test_hidden_new_rule_room_accepts_peng_claim_response_message() -> None:
    async def scenario() -> None:
...
def test_hidden_new_rule_room_accepts_chi_claim_response_message() -> None:
    async def scenario() -> None:
        server, websocket, response_websocket, game_state = await _started_room_with_responder()
        game_state.tiles_list = [33, 34]
...
def test_hidden_new_rule_room_accepts_gang_claim_response_message() -> None:
    async def scenario() -> None:
        server, websocket, response_websocket, game_state = await _started_room_with_responder()
        game_state.tiles_list = [33, 34]
        game_state.player_list[0].hand_tiles = [15]
...
def test_hidden_new_rule_room_accepts_concealed_kong_hand_action_message() -> None:
    async def scenario() -> None:
...
def test_hidden_new_rule_room_accepts_added_kong_pass_response_messages() -> None:
    async def scenario() -> None:
        server, websocket, response_websocket, game_state = await _started_room_with_responder()
        game_state.game_task.cancel()
        await asyncio.sleep(0)
...
def test_hidden_new_rule_room_accepts_robbing_kong_win_response_message() -> None:
    async def scenario() -> None:
        server, websocket, response_websocket, game_state = await _started_room_with_responder()
        game_state.game_task.cancel()
        await asyncio.sleep(0)
...
def test_hidden_new_rule_room_emits_final_settlement_reveal_payloads() -> None:
    async def scenario() -> None:
...
def test_hidden_new_rule_room_reconnect_after_end_reveals_final_state_by_manager() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        for player in game_state.player_list:
            player.hand_tiles = []
...
def test_hidden_new_rule_room_scripted_peng_cut_discard_win_continues_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
...
def test_hidden_new_rule_room_scripted_multi_ron_continues_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_hidden_new_rule_room_multi_ron_as_second_and_third_winners_ends_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_hidden_new_rule_room_late_action_after_end_does_not_mutate_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_hidden_new_rule_room_stale_response_after_window_change_is_ignored_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_hidden_new_rule_room_hu_and_pass_locks_only_passing_player_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_hidden_new_rule_room_hu_response_skips_peng_claim_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_hidden_new_rule_room_scripted_self_draw_win_continues_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
...
def test_hidden_new_rule_room_third_self_draw_win_ends_hand_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [34, 35]
        game_state.current_player_index = 0
...
def test_hidden_new_rule_room_final_discard_no_win_ends_by_wall_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
...
def test_hidden_new_rule_room_scripted_concealed_kong_cut_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_hidden_new_rule_room_scripted_added_kong_pass_cut_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_hidden_new_rule_room_scripted_added_kong_robbed_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def run() -> None:
    tests = [
        ...
        test_hidden_new_rule_room_start_requires_host,
        test_hidden_new_rule_room_rejects_non_member_ready_and_non_host_kick,
        test_room_manager_forces_hidden_new_rule_spectator_off,
        test_game_server_hidden_new_rule_create_wrapper_forces_spectator_off,
        test_hidden_new_rule_smoke_uses_game_server_connection_session,
        test_hidden_new_rule_smoke_uses_four_game_server_connection_sessions,
        test_room_router_handles_hidden_new_rule_create_message,
        test_hidden_new_rule_protocol_dispatch_create_start_cut_smoke,
        test_hidden_new_rule_room_can_start_game_state,
        test_hidden_new_rule_room_accepts_first_cut_message,
        test_hidden_new_rule_room_accepts_discard_win_response_message,
        test_hidden_new_rule_room_accepts_discard_pass_response_message,
        test_hidden_new_rule_room_accepts_peng_claim_response_message,
        test_hidden_new_rule_room_accepts_chi_claim_response_message,
        test_hidden_new_rule_room_accepts_gang_claim_response_message,
        test_hidden_new_rule_room_accepts_concealed_kong_hand_action_message,
        ...
        test_hidden_new_rule_room_multi_ron_as_second_and_third_winners_ends_by_messages,
        test_hidden_new_rule_room_late_action_after_end_does_not_mutate_by_messages,
        test_hidden_new_rule_room_stale_response_after_window_change_is_ignored_by_messages,
        test_hidden_new_rule_room_hu_and_pass_locks_only_passing_player_by_messages,
        test_hidden_new_rule_room_hu_response_skips_peng_claim_by_messages,
        test_hidden_new_rule_room_scripted_self_draw_win_continues_by_messages,
        test_hidden_new_rule_room_third_self_draw_win_ends_hand_by_messages,
        test_hidden_new_rule_room_final_discard_no_win_ends_by_wall_by_messages,
        test_hidden_new_rule_room_scripted_concealed_kong_cut_by_messages,
        test_hidden_new_rule_room_scripted_added_kong_pass_cut_by_messages,
        test_hidden_new_rule_room_scripted_added_kong_robbed_by_messages,
    ]
    for test in tests:
        test()
    print(f"new_rule room creation tests ok: {len(tests)} tests")
