...
from .init_tiles import init_jianzhong_tiles
...
JIANZHONG_MID_HU_ANIM_SECONDS = 0.5
...
class JianzhongGameState:
    """Live Jianzhong game state assembled from independent rule policies."""
    ...
    def __init__(
        ...
        db_manager: Any = None,
        gamestate_id: str = "jianzhong-test",
    ):
        ...
        self.room_rule = room_data.get("room_rule", "jianzhong")
        self.room_type = room_data.get("room_type", "custom")
        self.sub_rule = room_data.get("sub_rule", "jianzhong/standard")
        ...
        self.action_policy = JianzhongActionPolicy()
        self.settlement_policy = JianzhongSettlementPolicy()
    ...
    async def run_game_loop(self) -> None:
        ...
        try:
            ...
            logger.info("Jianzhong game loop cancelled, room_id=%s", self.room_id)
            raise
        except Exception as exc:
            logger.error("Jianzhong game loop failed, room_id=%s: %s", self.room_id, exc, exc_info=True)
            raise
    ...
    async def cleanup_game_state(self) -> None:
        ...
        if self.game_task and not self.game_task.done() and self.game_task is not current_task:
            ...
            try:
                ...
                logger.info("Jianzhong game loop cancelled during cleanup, room_id=%s", self.room_id)
            except Exception as exc:
                logger.error("Error while cancelling jianzhong game loop, room_id=%s: %s", self.room_id, exc)
    ...
    def schedule_bot_actions(self) -> None:
        from .bot import jianzhong_bot_action
    ...
    def emit_mid_hand_hu_payloads(
        ...
        *,
        ...
        self.outbound_payloads.append({
            "type": "gamestate/jianzhong/_internal_mid_hand_hu_delay",
            "delay_seconds": JIANZHONG_MID_HU_ANIM_SECONDS,
        })
        return payloads
    ...
    def persist_game_record(self) -> Optional[str]:
        ...
        if self.db_manager is None or not self.game_record:
            return None
        try:
            return self.db_manager.store_jianzhong_game_record(
                self.game_record,
                self.player_list,
                self.room_type,
                f"{self.max_round}/4",
            ...
            logger.warning(
                "Jianzhong replay persistence failed, room_id=%s: %s",
                self.room_id,
                exc,
                exc_info=True,
            )
    ...
    def _apply_discard_claim(self, player: JianzhongPlayer, tile: int, action: str) -> tuple[str, list[int]]:
        if action == "peng":
