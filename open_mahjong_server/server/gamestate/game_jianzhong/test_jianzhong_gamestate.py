...
from server.gamestate.game_jianzhong.boardcast import final_settlement_payloads
...
def test_apply_action_results_self_draw_win_emits_deferred_show_result() -> None:
    ...
    deal_payloads = [
        payload
        for payload in game.outbound_payloads
        if payload.get("type") == "gamestate/jianzhong/do_action"
...
def test_apply_action_results_third_self_draw_win_with_zero_target_ends_hand() -> None:
    ...
    assert not any(
        payload.get("type") == "gamestate/jianzhong/do_action"
        and payload.get("do_action_info", {}).get("action_list") == ["deal_tile"]
        for payload in game.outbound_payloads
    )
...
def test_apply_action_results_mid_multi_ron_marks_recycle_only_on_last_panel() -> None:
    ...
    viewer0_payloads = [
        ...
        or payload.get("type") == "gamestate/jianzhong/_internal_mid_hand_hu_delay"
    ]
    delay_indexes = [
        i for i, payload in enumerate(viewer0_payloads)
        if payload.get("type") == "gamestate/jianzhong/_internal_mid_hand_hu_delay"
    ]
    panel_indexes = [
        i for i, payload in enumerate(viewer0_payloads)
        if payload.get("type") == "gamestate/jianzhong/show_result"
...
def test_apply_action_results_terminal_discard_win_emits_tile_move_then_final_settlement() -> None:
    ...
    assert not any(
        payload.get("type") == "gamestate/jianzhong/show_result"
        and payload.get("show_result_info", {}).get("defer_score_settlement") is True
...
def _setup_context_fan_houtei(game: JianzhongGameState) -> None:
    for player in game.player_list:
        player.hand_tiles = []
        player.waiting_tiles = set()
    game.tiles_list = []
...
def _setup_context_fan_chankan(game: JianzhongGameState) -> None:
    for player in game.player_list:
        player.hand_tiles = []
        player.waiting_tiles = set()
        player.combination_tiles = []
...
def test_visible_action_payloads_are_addressed_to_each_viewer() -> None:
    ...
    assert all(payload["type"] == "gamestate/jianzhong/do_action" for payload in payloads)
...
def test_run_game_loop_starts_and_can_be_cancelled() -> None:
    async def scenario() -> None:
        ...
        assert game.game_status == "waiting_hand_action"
        assert game.waiting_players_list
        assert any(payload["type"] == "gamestate/jianzhong/broadcast_hand_action" for payload in game.outbound_payloads)
...
def test_run_game_loop_sends_game_start_before_first_hand_action() -> None:
    async def scenario() -> None:
        ...
        assert payload_types[:4] == ["gamestate/jianzhong/game_start"] * 4
        assert "gamestate/jianzhong/broadcast_hand_action" in payload_types[4:]
...
def test_round_ready_phase_waits_for_human_and_marks_bots_ready() -> None:
    async def scenario() -> None:
        ...
        ready_payload = next(
            ...
            if payload["type"] == "gamestate/jianzhong/ready_status"
        )
...
def test_final_round_waits_for_ready_before_game_end() -> None:
    async def scenario() -> None:
        ...
        for _ in range(100):
            payload_types = [payload["type"] for payload in game.outbound_payloads]
            if "gamestate/jianzhong/ready_status" in payload_types:
                break
            await asyncio.sleep(0.01)

        payload_types = [payload["type"] for payload in game.outbound_payloads]
        assert "gamestate/jianzhong/ready_status" in payload_types
        assert "gamestate/jianzhong/game_end" not in payload_types
        ...
        payload_types = [payload["type"] for payload in game.outbound_payloads]
        assert payload_types[-4:] == ["gamestate/jianzhong/game_end"] * 4
...
def test_recording_initializes_standard_game_record_round() -> None:
    ...
    assert title["rule"] == "jianzhong"
    assert title["sub_rule"] == "jianzhong/standard"
...
def test_persist_game_record_uses_jianzhong_database_adapter() -> None:
    calls = []
    ...
    game = JianzhongGameState(db_manager=FakeDatabase())
    game.start_game_recording()
...
def test_open_action_window_sets_live_action_fields() -> None:
    ...
    assert game.outbound_payloads[-1]["type"] == "gamestate/jianzhong/broadcast_hand_action"
...
def test_flush_outbound_payloads_sends_to_connected_player() -> None:
    async def scenario() -> None:
        ...
        assert websocket.sent[0]["type"] == "gamestate/jianzhong/broadcast_hand_action"
...
def test_send_payload_to_player_without_connection_keeps_outbox_fallback() -> None:
    async def scenario() -> None:
        ...
        sent = await game.send_payload_to_player(0, {"type": "test/jianzhong"})

        assert not sent
        assert len(game.outbound_payloads) == before + 1
        assert game.outbound_payloads[-1]["type"] == "test/jianzhong"
...
def test_player_reconnect_sends_game_start_then_pending_action_payload() -> None:
    async def scenario() -> None:
        ...
        assert [payload["type"] for payload in websocket.sent[-2:]] == [
            "gamestate/jianzhong/game_start",
            "gamestate/jianzhong/ask_other_action",
        ]
...
def test_complete_game_lifecycle_sends_ready_game_end_and_finishes_room() -> None:
    async def scenario() -> None:
        ...
        payload_types = [payload["type"] for payload in game.outbound_payloads]
        assert payload_types[-8:-4] == ["gamestate/jianzhong/ready_status"] * 4
        assert payload_types[-4:] == ["gamestate/jianzhong/game_end"] * 4
...
def run() -> None:
    tests = [
        ...
        test_persist_game_record_uses_jianzhong_database_adapter,
    ...
    for test in tests:
        test()
    print(f"jianzhong gamestate tests ok: {len(tests)} tests")
