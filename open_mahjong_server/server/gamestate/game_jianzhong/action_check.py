...
def refresh_waiting_tiles(game_state, player_index: int, *, exclude_last_tile: bool = False) -> set[int]:
    """Refresh a player's waiting-tile cache from the Jianzhong calculator."""
    player = game_state.player_list[player_index]
    hand_tiles = player.hand_tiles[:-1] if exclude_last_tile else player.hand_tiles
    player.waiting_tiles = game_state.calculation_service.Jianzhong_tingpai_check(
        hand_tiles,
...
class JianzhongActionPolicy:
    """Replaceable legal-action module used by the Jianzhong composition."""
