...
from server.gamestate.game_jianzhong import JianzhongActionPolicy, JianzhongGameState, JianzhongSettlementPolicy
from server.gamestate.game_jianzhong.boardcast import game_info_payload
...
from server.room.room_validators import JianzhongRoomValidator
...
def test_policy_can_be_composed_with_non_jianzhong_players() -> None:
    # The policy only needs player_index/is_hu, so a future Guobiao state can
    # adopt it without inheriting JianzhongGameState or its scoring/actions.
...
def test_room_validation_accepts_three_public_modes() -> None:
    ...
    for mode in HandEndMode:
        validated = JianzhongRoomValidator(**common, hand_end_mode=mode.value)
...
def test_first_second_and_third_win_modes_end_at_their_targets() -> None:
    for mode, target in (("first_win", 1), ("second_win", 2), ("third_win", 3)):
        game = JianzhongGameState(room_data=room_data(mode))
...
def test_hand_end_target_does_not_truncate_same_event_multi_ron() -> None:
    first = JianzhongGameState(room_data=room_data("first_win"))
    ...
    second = JianzhongGameState(room_data=room_data("second_win"))
...
def test_jianzhong_composes_public_flow_rule_scoring_and_presentation() -> None:
    game = JianzhongGameState(room_data=room_data("second_win"))
    assert game.winner_target == 2
    assert isinstance(game.rule_composition.actions, JianzhongActionPolicy)
    assert isinstance(game.settlement_policy, JianzhongSettlementPolicy)
...
def test_game_info_declares_capabilities_instead_of_requiring_rule_name_checks() -> None:
    game = JianzhongGameState(room_data=room_data("first_win"))
...
def run() -> None:
    tests = [
        ...
        test_policy_can_be_composed_with_non_jianzhong_players,
        ...
        test_jianzhong_composes_public_flow_rule_scoring_and_presentation,
