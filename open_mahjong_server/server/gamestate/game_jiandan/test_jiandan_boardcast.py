...
def test_final_settlement_without_winners_emits_unity_draw_result() -> None:
    ...
    assert payload["type"] == "gamestate/jiandan/show_result"
...
def test_ask_action_payload_includes_unity_bridge_fields() -> None:
    ...
    assert hand_payload["game_info"]["room_rule"] == "jiandan"
    assert hand_payload["game_info"]["sub_rule"] == "jiandan/standard"
    assert hand_payload["game_info"]["self_hand_tiles"] == game.player_list[0].hand_tiles
    assert hand_payload["type"] == "gamestate/jiandan/broadcast_hand_action"
    ...
    assert other_payload["type"] == "gamestate/jiandan/ask_other_action"
...
def test_game_start_payload_uses_viewer_safe_state() -> None:
    ...
    assert payload["type"] == "gamestate/jiandan/game_start"
...
def test_ready_status_payload_uses_existing_ready_shape() -> None:
    ...
    assert payload["type"] == "gamestate/jiandan/ready_status"
...
def test_game_end_payload_uses_existing_game_end_shape() -> None:
    ...
    assert payload["type"] == "gamestate/jiandan/game_end"
    assert payload["player_index"] == 0
