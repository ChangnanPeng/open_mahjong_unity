...
from server.gamestate.game_jiandan.action_check import (
...
def prepared_game() -> JiandanGameState:
...
def test_refresh_waiting_tiles_from_jiandan_calculator() -> None:
    game = prepared_game()
...
def run() -> None:
    tests = [
        ...
        test_refresh_waiting_tiles_from_jiandan_calculator,
    ]
    for test in tests:
        test()
    print(f"jiandan action_check tests ok: {len(tests)} tests")
