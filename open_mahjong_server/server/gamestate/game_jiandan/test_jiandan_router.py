...
def _server_with_game(game: JiandanGameState):
...
def test_jiandan_cut_tile_route_queues_cut_action() -> None:
...
def test_jiandan_cut_tile_route_ignores_stale_action_tick() -> None:
    async def scenario() -> None:
        game = JiandanGameState()
        server = _server_with_game(game)
        game.player_list[0].hand_tiles = [11, 12, 13]
...
def test_jiandan_send_action_route_queues_response_action() -> None:
    async def scenario() -> None:
        game = JiandanGameState()
        server = _server_with_game(game)
...
def test_jiandan_route_rejects_non_jiandan_state() -> None:
    async def scenario() -> None:
        game = JiandanGameState()
        server = _server_with_game(game)
...
def run() -> None:
    tests = [
        test_jiandan_cut_tile_route_queues_cut_action,
        test_jiandan_cut_tile_route_ignores_stale_action_tick,
        test_jiandan_send_action_route_queues_response_action,
        test_jiandan_route_rejects_non_jiandan_state,
    ]
    for test in tests:
        test()
    print(f"jiandan router tests ok: {len(tests)} tests")
