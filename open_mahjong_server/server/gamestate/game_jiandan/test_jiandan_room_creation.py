...
async def _started_room_with_responder():
    ...
    response = await server.room_manager.create_Jiandan_room(
        "conn-101",
        "jiandan smoke",
        1,
        "",
    ...
    assert isinstance(game_state, JiandanGameState)
...
async def _send_host_cut(server, websocket, game_state: JiandanGameState, tile: int) -> None:
...
async def _wait_for_status(game_state: JiandanGameState, status: str, player_index: int | None = None) -> None:
...
def _force_final_configured_round(game_state: JiandanGameState) -> None:
    game_state.current_round = game_state.max_round * 4
    game_state.round_index = game_state.current_round


def test_room_manager_creates_public_jiandan_room() -> None:
    async def scenario() -> None:
        ...
        room_info = server.room_manager.rooms[response.room_info["room_id"]]
        assert room_info["room_rule"] == "jiandan"
        assert room_info["sub_rule"] == "jiandan/standard"
...
def test_jiandan_room_is_listed_and_syncable() -> None:
    async def scenario() -> None:
...
def test_jiandan_room_allows_join_by_room_id() -> None:
    async def scenario() -> None:
        server, _websocket, _player = _server()
...
def test_jiandan_room_start_requires_host() -> None:
    async def scenario() -> None:
        server, _websocket, _player = _server()
        _add_connected_player(server, 102, "conn-102")
...
def test_jiandan_room_rejects_non_member_ready_and_non_host_kick() -> None:
    async def scenario() -> None:
        server, _websocket, _player = _server()
        _add_connected_player(server, 102, "conn-102")
...
def test_room_manager_preserves_jiandan_spectator_setting() -> None:
    async def scenario() -> None:
        server, _websocket, _player = _server()
...
def test_game_server_jiandan_create_wrapper_preserves_spectator_setting() -> None:
    async def scenario() -> None:
...
def test_jiandan_smoke_uses_game_server_connection_session() -> None:
    async def scenario() -> None:
        server = GameServer()
        server.db_manager = FakeDbManager()
        server.match_manager = FakeMatchManager()
...
def test_jiandan_smoke_uses_four_game_server_connection_sessions() -> None:
    async def scenario() -> None:
        server = GameServer()
        server.db_manager = FakeDbManager()
        server.match_manager = FakeMatchManager()
        ...
        await _wait_for_condition(
            lambda: any(
                payload["type"] == "gamestate/jiandan/broadcast_hand_action"
...
def test_room_router_handles_jiandan_create_message() -> None:
    async def scenario() -> None:
        server, websocket, player = _server()
...
def test_jiandan_protocol_dispatch_create_start_cut_smoke() -> None:
    async def scenario() -> None:
        server, websocket, player = _server()
...
def test_jiandan_room_can_start_game_state() -> None:
    async def scenario() -> None:
        server, websocket, player = _server()
        response = await server.room_manager.create_Jiandan_room(
            "conn-101",
        ...
        game_state = server.gamestate_manager.get_game_state_by_room_id(room_info["room_id"])
        assert isinstance(game_state, JiandanGameState)
        assert game_state.room_rule == "jiandan"
...
def test_jiandan_room_accepts_first_cut_message() -> None:
    async def scenario() -> None:
        server, websocket, _player = _server()
        response = await server.room_manager.create_Jiandan_room(
            "conn-101",
...
def test_jiandan_room_accepts_discard_win_response_message() -> None:
    async def scenario() -> None:
        server, websocket, _player = _server()
        response_websocket, response_player = _add_connected_player(server, 102, "conn-102")
        response = await server.room_manager.create_Jiandan_room(
        ...
        await handle_gamestate_message(
            ...
            "conn-102",
            {
                "type": "gamestate/jiandan/send_action",
                "gamestate_id": game_state.gamestate_id,
                "action": "hu",
...
def test_jiandan_room_accepts_discard_pass_response_message() -> None:
    async def scenario() -> None:
        server, websocket, _player = _server()
        response_websocket, response_player = _add_connected_player(server, 102, "conn-102")
        response = await server.room_manager.create_Jiandan_room(
...
def test_jiandan_room_accepts_peng_claim_response_message() -> None:
    async def scenario() -> None:
...
def test_jiandan_room_accepts_chi_claim_response_message() -> None:
    async def scenario() -> None:
        server, websocket, response_websocket, game_state = await _started_room_with_responder()
        game_state.tiles_list = [33, 34]
...
def test_jiandan_room_accepts_gang_claim_response_message() -> None:
    async def scenario() -> None:
        server, websocket, response_websocket, game_state = await _started_room_with_responder()
        game_state.tiles_list = [33, 34]
        game_state.player_list[0].hand_tiles = [15]
...
def test_jiandan_room_accepts_concealed_kong_hand_action_message() -> None:
    async def scenario() -> None:
...
def test_jiandan_room_accepts_added_kong_pass_response_messages() -> None:
    async def scenario() -> None:
        server, websocket, response_websocket, game_state = await _started_room_with_responder()
        game_state.game_task.cancel()
        await asyncio.sleep(0)
...
def test_jiandan_room_accepts_robbing_kong_win_response_message() -> None:
    async def scenario() -> None:
        server, websocket, response_websocket, game_state = await _started_room_with_responder()
        game_state.game_task.cancel()
        await asyncio.sleep(0)
...
def test_jiandan_room_emits_final_settlement_reveal_payloads() -> None:
    async def scenario() -> None:
...
def test_jiandan_room_reconnect_restores_game_start_and_pending_action_by_manager() -> None:
    async def scenario() -> None:
        ...
        game_start_payloads = [
            ...
            if payload.get("type") == "gamestate/jiandan/game_start"
        ]
        ...
        pending_action_payloads = [
            ...
            if item.get("type") == "gamestate/jiandan/ask_other_action"
        ]
...
def test_jiandan_room_reconnect_after_end_reveals_final_state_by_manager() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        for player in game_state.player_list:
            player.hand_tiles = []
        ...
        player_zero = payload["game_info"]["players_info"][0]
        player_one = payload["game_info"]["players_info"][1]
        assert payload["game_info"]["room_rule"] == "jiandan"
        assert player_zero["hand_tiles"] == [11, 12, 13]
        assert player_zero["combination_tiles"] == ["G15"]
        assert player_one["hand_tiles"] == [21, 22, 23]
...
def test_jiandan_room_scripted_peng_cut_discard_win_continues_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
...
def test_jiandan_room_scripted_multi_ron_continues_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_jiandan_room_multi_ron_as_second_and_third_winners_ends_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_jiandan_room_late_action_after_end_does_not_mutate_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_jiandan_room_stale_response_after_window_change_is_ignored_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_jiandan_room_hu_and_pass_locks_only_passing_player_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_jiandan_room_hu_response_skips_peng_claim_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_jiandan_room_scripted_self_draw_win_continues_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
...
def test_jiandan_room_third_self_draw_win_ends_hand_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [34, 35]
        game_state.current_player_index = 0
...
def test_jiandan_room_final_discard_no_win_ends_by_wall_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
...
def test_jiandan_room_scripted_concealed_kong_cut_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_jiandan_room_scripted_added_kong_pass_cut_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def test_jiandan_room_scripted_added_kong_robbed_by_messages() -> None:
    async def scenario() -> None:
        server, websockets, game_state = await _started_room_with_connected_players()
        game_state.tiles_list = [33, 34]
        game_state.current_player_index = 0
...
def run() -> None:
    tests = [
        test_room_manager_creates_public_jiandan_room,
        test_room_router_persists_second_winner_flow_option,
        test_jiandan_room_is_listed_and_syncable,
        test_jiandan_room_allows_join_by_room_id,
        test_jiandan_room_start_requires_host,
        test_jiandan_room_rejects_non_member_ready_and_non_host_kick,
        test_room_manager_preserves_jiandan_spectator_setting,
        test_game_server_jiandan_create_wrapper_preserves_spectator_setting,
        test_jiandan_smoke_uses_game_server_connection_session,
        test_jiandan_smoke_uses_four_game_server_connection_sessions,
        test_room_router_handles_jiandan_create_message,
        test_jiandan_protocol_dispatch_create_start_cut_smoke,
        test_jiandan_room_can_start_game_state,
        test_jiandan_room_accepts_first_cut_message,
        test_jiandan_room_accepts_discard_win_response_message,
        test_jiandan_room_accepts_discard_pass_response_message,
        test_jiandan_room_accepts_peng_claim_response_message,
        test_jiandan_room_accepts_chi_claim_response_message,
        test_jiandan_room_accepts_gang_claim_response_message,
        test_jiandan_room_accepts_concealed_kong_hand_action_message,
        test_jiandan_room_accepts_added_kong_pass_response_messages,
        test_jiandan_room_accepts_robbing_kong_win_response_message,
        test_jiandan_room_emits_final_settlement_reveal_payloads,
        test_jiandan_room_reconnect_restores_game_start_and_pending_action_by_manager,
        test_jiandan_room_reconnect_after_end_reveals_final_state_by_manager,
        test_jiandan_room_scripted_peng_cut_discard_win_continues_by_messages,
        test_jiandan_room_scripted_multi_ron_continues_by_messages,
        test_jiandan_room_multi_ron_as_second_and_third_winners_ends_by_messages,
        test_jiandan_room_late_action_after_end_does_not_mutate_by_messages,
        test_jiandan_room_stale_response_after_window_change_is_ignored_by_messages,
        test_jiandan_room_hu_and_pass_locks_only_passing_player_by_messages,
        test_jiandan_room_hu_response_skips_peng_claim_by_messages,
        test_jiandan_room_scripted_self_draw_win_continues_by_messages,
        test_jiandan_room_third_self_draw_win_ends_hand_by_messages,
        test_jiandan_room_final_discard_no_win_ends_by_wall_by_messages,
        test_jiandan_room_scripted_concealed_kong_cut_by_messages,
        test_jiandan_room_scripted_added_kong_pass_cut_by_messages,
        test_jiandan_room_scripted_added_kong_robbed_by_messages,
    ]
    for test in tests:
        test()
