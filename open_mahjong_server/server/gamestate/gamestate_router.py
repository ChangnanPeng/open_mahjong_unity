...
async def handle_gamestate_message(game_server, Connect_id: str, message: dict, websocket):
    ...
    if message_type == "gamestate/GB/cut_tile":
        ...
        await handle_jiandan_cut_tile(game_server, Connect_id, message, websocket)
    elif message_type == "gamestate/jiandan/send_action":
        await handle_jiandan_send_action(game_server, Connect_id, message, websocket)
...
async def handle_jiandan_cut_tile(game_server, Connect_id: str, message: dict, websocket):
    """Route a Jiandan discard request to its action adapter."""
    try:
        gamestate_id = message.get("gamestate_id")
        if not gamestate_id:
            logger.warning(f"jiandan cut_tile missing gamestate_id: {message}")
            return

        game_state = game_server.gamestate_manager.get_game_state_by_gamestate_id(gamestate_id)
        if not game_state:
            logger.warning(f"jiandan gamestate_id not found: {gamestate_id}")
            return
        if getattr(game_state, "room_rule", None) != "jiandan":
            logger.warning(f"jiandan cut_tile routed to a different rule: {gamestate_id}")
            return
        ...
        logger.error(f"handle jiandan cut_tile failed: {e}", exc_info=True)


async def handle_jiandan_send_action(game_server, Connect_id: str, message: dict, websocket):
    """Route a Jiandan non-discard action request to its action adapter."""
    try:
        gamestate_id = message.get("gamestate_id")
        if not gamestate_id:
            logger.warning(f"jiandan send_action missing gamestate_id: {message}")
            return
        ...
        if not game_state:
            ...
            return
        if getattr(game_state, "room_rule", None) != "jiandan":
            logger.warning(f"jiandan send_action routed to a different rule: {gamestate_id}")
            return
        ...
        logger.error(f"handle jiandan send_action failed: {e}", exc_info=True)
