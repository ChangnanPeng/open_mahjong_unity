...
from .game_jianzhong.JianzhongGameState import JianzhongGameState
...
class GameStateManager:
    ...
    def __init__(self, game_server):
        ...
        self.room_id_to_JianzhongGameState: Dict[str, JianzhongGameState] = {}
    ...
    async def start_game(self, Connect_id: str, room_id: str) -> Optional[Response]:
        ...
        if room_rule == "guobiao":
            ...
            try:
                ...
                return Response(type="error_message", success=False, message=f"启动游戏失败: {str(e)}")
        elif room_rule == "jianzhong":
            try:
                gamestate_id = str(uuid.uuid4())

                game_state = JianzhongGameState(
                    self.game_server,
                    room_data,
                    self.game_server.calculation_service,
                    self.game_server.db_manager,
                ...
                self.room_id_to_JianzhongGameState[room_id] = game_state
                self.gamestate_id_to_game_state[gamestate_id] = game_state
                ...
                game_state.game_task = asyncio.create_task(game_state.run_game_loop())
                logger.info(f"jianzhong game started, room_id={room_id}, gamestate_id={gamestate_id}")
            except Exception as e:
                logger.error(f"failed to start jianzhong game, room_id: {room_id}, error: {e}", exc_info=True)
                room_data["is_game_running"] = False
                if room_id in self.room_id_to_JianzhongGameState:
                    game_state = self.room_id_to_JianzhongGameState[room_id]
                    if hasattr(game_state, 'gamestate_id') and game_state.gamestate_id in self.gamestate_id_to_game_state:
                        del self.gamestate_id_to_game_state[game_state.gamestate_id]
                    del self.room_id_to_JianzhongGameState[room_id]
                return Response(type="error_message", success=False, message=f"启动游戏失败: {str(e)}")
        else:
    ...
    def get_game_state_by_room_id(self, room_id: str) -> Optional[Any]:
        ...
        elif room_id in self.room_id_to_JianzhongGameState:
            return self.room_id_to_JianzhongGameState.get(room_id)
        return None
    ...
    async def cleanup_game_state_complete(self, gamestate_id: str = None, room_id: str = None):
        ...
        elif game_state.room_id in self.room_id_to_JianzhongGameState:
            del self.room_id_to_JianzhongGameState[game_state.room_id]
