    ...
    async def start_game(self, Connect_id: str, room_id: str) -> Optional[Response]:
        ...
        if room_rule == "guobiao":
            ...
            try:
                ...
                game_state.game_task = asyncio.create_task(game_state.run_game_loop())
                logger.info(f"jiandan game started, room_id={room_id}, gamestate_id={gamestate_id}")
            except Exception as e:
                logger.error(f"failed to start jiandan game, room_id: {room_id}, error: {e}", exc_info=True)
                room_data["is_game_running"] = False
