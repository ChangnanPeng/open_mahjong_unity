...
def test_jiandan_record_uses_existing_replay_tables() -> None:
    ...
    game_id = store_jiandan_game_record(database, record, players, "custom", "1/4")
    ...
    assert all(
        params[6:10] == ("jiandan", "jiandan/standard", "1/4", "custom")
        for params in player_inserts
    )
...
def test_jiandan_record_skips_games_with_bots() -> None:
    database = FakeDatabase()
    players = [_player(1, 0)] + [_player(102 + index, index + 1) for index in range(3)]

    assert store_jiandan_game_record(
        database,
        {"game_title": {}},
        players,
        "custom",
...
def run() -> None:
    tests = [
        test_jiandan_record_uses_existing_replay_tables,
        test_jiandan_record_skips_games_with_bots,
    ]
    for test in tests:
        test()
    print(f"jiandan database smoke tests ok: {len(tests)} tests")
