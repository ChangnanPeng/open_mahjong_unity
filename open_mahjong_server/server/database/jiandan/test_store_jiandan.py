...
def test_jiandan_record_uses_shared_tables_and_rule_identity() -> None:
    ...
    game_id = store_jiandan_game_record(
        database,
        record,
        players,
        "custom",
    ...
    assert all(params[6:10] == ("jiandan", "jiandan/standard", "1/4", "custom") for params in player_inserts)
...
def run() -> None:
    tests = [
        test_jiandan_record_uses_shared_tables_and_rule_identity,
        test_jiandan_record_skips_games_with_bots,
    ]
    for test in tests:
        test()
    print(f"jiandan database tests ok: {len(tests)} tests")
