...
def test_service_basic_score() -> None:
    ...
    points, fan_names = service.Jianzhong_hepai_check(
...
def test_service_detail_fan_ids_and_context() -> None:
    service = GameCalculationService()
    detail = service.Jianzhong_hepai_detail(
...
def test_service_tingpai_check() -> None:
    service = GameCalculationService()
    waits = service.Jianzhong_tingpai_check(
...
def run() -> None:
    ...
    for test in tests:
        test()
    print(f"jianzhong service smoke tests ok: {len(tests)} tests")
