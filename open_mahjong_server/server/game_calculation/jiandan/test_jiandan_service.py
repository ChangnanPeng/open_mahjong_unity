...
def test_service_basic_score() -> None:
    ...
    points, fan_names = service.Jiandan_hepai_check(
...
def test_service_detail_fan_ids_and_context() -> None:
    service = GameCalculationService()
    detail = service.Jiandan_hepai_detail(
...
def test_service_tingpai_check() -> None:
    service = GameCalculationService()
    waits = service.Jiandan_tingpai_check(
...
def run() -> None:
    ...
    for test in tests:
        test()
    print(f"jiandan service smoke tests ok: {len(tests)} tests")
